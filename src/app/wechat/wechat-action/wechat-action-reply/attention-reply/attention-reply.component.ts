...
@Component({
  selector: 'app-attention-reply',
  templateUrl: './attention-reply.component.html',
  styleUrls: ['./attention-reply.component.scss']
})
export class AttentionReplyComponent extends WechatComponent implements OnInit {
  ...
  // 获取配置变量
  // private getUrl = '/subscribe-msgs/' + this.localData.id;
  private dataUrl = '/subscribe-msgs';
    ...
    return this.http.post(this.dataUrl, this.formModel.value).subscribe(
      result => {
        if (result) {
          ...
          this.router.navigate(['wechat/wechat-action/reply/reply-index']);
        }
      },
      err => {
