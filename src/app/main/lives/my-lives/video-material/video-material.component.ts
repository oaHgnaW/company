...
export class VideoMaterialComponent implements OnInit {
   ...
   */
  getVideosNum() {
    this.videoService.getVideosMaterialNum().subscribe(
      res => {
        this.videosCount = res;
      });
  }
  ...
  getVideosList(params = {}) {
    ...
    this.videoService.getVideoList(this.params).subscribe(
      res => {
   ...
   */
  deleteVideos() {
    this.videoService.deleteVideo(this.deleteId).subscribe(
      res => {
